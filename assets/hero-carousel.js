...
// Scroll-snap carousel controls: arrows with wrap-around, pagination dots,
// and optional autoplay that pauses on hover, focus, interaction, when the
// section is off screen, and permanently when the visitor hits pause.
// Swiping works without this file thanks to native scroll-snap.
//
// Scheduling & VIP: The carousel automatically adapts to scheduled slides
// and VIP ordering based on customer tags.
...
(() => {
  ...
  const initCarousel = (carousel) => {
    ...
    dots.forEach((dot) => {
      dot.addEventListener('click', () => goTo(Number(dot.dataset.hcDot)));
    });
    ...
    let timer = null;
    let pausedByUser = false; // pause button: permanent until pressed again
    let hovered = false;
    let focused = false;
    let visible = true;
    ...
    const setPlaybackState = (paused) => {
      ...
      // Announce slide changes only while not auto-rotating
  ...
  // Scroll to the slide the merchant selects in the editor sidebar
  document.addEventListener('shopify:block:select', (event) => {
    const slide = event.target.closest ? event.target.closest('[data-hc-slide]') : null;
    const target = slide || (event.target.querySelector && event.target.querySelector('[data-hc-slide]'));
    if (!target) return;
    const scroller = target.closest('[data-hc-scroller]');
    if (scroller) {
