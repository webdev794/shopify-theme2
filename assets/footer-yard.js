...
(function () {
  ...
  var RABBIT_SVG =
    ...
    '<g class="footer-yard__rabbit-ears">' +
    ...
    '</g>' +
    '<circle class="footer-yard__rabbit-tail" cx="14" cy="38" r="6" fill="#faf4ea"/>' +
    ...
    '<g class="footer-yard__rabbit-head">' +
  ...
  var BUTTERFLY_SVG =
    ...
    '<g class="footer-yard__wing-left">' +
    ...
    '</g>' +
    '<g class="footer-yard__wing-right">' +
  ...
  function mount() {
    if (document.getElementById('footer-yard')) return;

    var footerGroup = document.querySelector(
      '.shopify-section-group-footer-group'
    );
    if (!footerGroup || !footerGroup.parentNode) return;

    var yard = document.createElement('div');
    yard.id = 'footer-yard';
    yard.setAttribute('aria-hidden', 'true');
    yard.innerHTML =
      '<div class="footer-yard__ground"></div>' +
      '<div class="footer-yard__plate">' +
      PLATE_SVG +
      '</div>' +
      '<div class="footer-yard__water-bowl">' +
      WATER_BOWL_SVG +
      '</div>' +
      '<div class="footer-yard__dog" data-dog-mode="run">' +
      DOG_SVG +
      '</div>' +
      '<div class="footer-yard__cat is-running">' +
      CAT_SVG +
      '</div>' +
      '<div class="footer-yard__bird">' +
      BIRD_SVG +
      '</div>' +
      '<div class="footer-yard__rabbit">' +
      RABBIT_SVG +
      '</div>' +
      '<div class="footer-yard__butterfly">' +
      BUTTERFLY_SVG +
      '</div>' +
      '<div class="footer-yard__ball">' +
      BALL_SVG +
      '</div>';

    footerGroup.parentNode.insertBefore(yard, footerGroup);
    start(yard);
  }

  function start(yard) {
    var dogEl = yard.querySelector('.footer-yard__dog');
    var catEl = yard.querySelector('.footer-yard__cat');
    var birdEl = yard.querySelector('.footer-yard__bird');
    var rabbitEl = yard.querySelector('.footer-yard__rabbit');
    var butterflyEl = yard.querySelector('.footer-yard__butterfly');
    var ballEl = yard.querySelector('.footer-yard__ball');
    var plateEl = yard.querySelector('.footer-yard__plate');
    var waterBowlEl = yard.querySelector('.footer-yard__water-bowl');
    ...
    function setCatVisual(s) {
      var cls =
        s === 'eating'
          ...
          ? 'is-startled'
          : 'is-running';
      catEl.className = 'footer-yard__cat ' + cls;
    }
    ...
    function setRabbitVisual(s) {
      ...
      rabbitEl.className = 'footer-yard__rabbit ' + cls;
    }
    ...
    function setBirdVisual(s) {
      var cls =
        s === 'landed-eating'
          ...
          ? 'is-perched'
          : 'is-flying';
      birdEl.className = 'footer-yard__bird' + (s === 'hidden' ? '' : ' ' + cls);
        ...
        function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting) {
              startLoop();
              window.PetlioFooterYardActive = true;
              document.body.classList.add('petlio-footer-yard-active');
            } else {
              stopLoop();
              window.PetlioFooterYardActive = false;
              document.body.classList.remove('petlio-footer-yard-active');
            }
          });
        },
